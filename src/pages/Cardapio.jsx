...
const CATEGORIAS = ['Prato Principal', 'Acompanhamento', 'Bebida', 'Sobremesa', 'Entrada']
const VAZIO = { nome: '', categoria: 'Prato Principal', descricao: '', preco: '' }
...
export default function Cardapio() {
  ...
  function salvar() {
    ...
    adicionarItemCardapio({ ...form, preco: parseFloat(form.preco) })
    setForm(VAZIO)
      ...
      {mostrarForm && (
        <div className="bg-white border border-amber-200 rounded-xl p-5 mb-5 shadow-sm">
          ...
          <div className="flex gap-2 mt-4">
            ...
            <button onClick={() => { setForm(VAZIO); setMostrarForm(false) }} className="flex items-center gap-1 bg-gray-100 hover:bg-gray-200 text-gray-700 px-4 py-2 rounded-lg text-sm font-medium transition-colors">
              <X size={15} /> Cancelar
            </button>
          </div>
        </div>
      ...
      {Object.keys(porCategoria).length === 0 && indisponiveis.length === 0 ? (
        ...
        <div className="space-y-5">
          {Object.entries(porCategoria).map(([cat, itens]) => (
            <div key={cat}>
              <h2 className="text-xs font-bold text-amber-700 uppercase tracking-wider mb-2">{cat}</h2>
